...
## 2026-07-01 日期快速跳轉選擇器
- 課程排程頂部日期文字升級為可點擊的原生 Date Picker，可直接跳轉跨月或指定日期。
- 透明 `input[type=date]` 覆蓋整個中間日期區塊，保留乾淨文字外觀並加入 Calendar 圖示提示。
- 日期輸入與 `selectedDate` 雙向綁定；若瀏覽器回傳空值則保留目前日期，避免查詢狀態失效。

## 2026-07-01 一週課程總表位置資訊
- 一週總表每堂課新增泳池與水道標記，位置會顯示在學生與課型資訊後方。
- 25m 大池有水道時顯示 `[25M 大池 第N道]`，缺少水道時顯示 `[25M 大池]`。
- 小池不附加水道，固定顯示 `[小池]`；歷史資料缺少泳池類型時顯示安全 fallback。
